...
def recuperer_vraie_meteo(lat, lon, date_str):
    ...
    try:
        ...
        with urllib.request.urlopen(req_om, timeout=15) as response:
...
def recuperer_donnees_balise_reelles(balise_id):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=8) as response:
            ...
            # Recherche textuelle ultra-ciblée : évite les index glissants
            match_moyen = re.search(r"Vent\s+moyen\s*:\s*([\d\.,]+)", html_content, re.IGNORECASE)
            match_maxi = re.search(r"Vent\s+maxi\s*:\s*([\d\.,]+)", html_content, re.IGNORECASE)
            
            if match_moyen and match_maxi:
                vent_moyen = float(match_moyen.group(1).replace(',', '.'))
                vent_max = float(match_maxi.group(1).replace(',', '.'))
            else:
                # Fallback agnostique si les textes ont bougé : on isole les km/h
                ...
                if len(valeurs) >= 3:
                    ...
                    vent_max = valeurs[2] # On saute l'index 1 (le mini, même s'il vaut 0)
                elif len(valeurs) == 2:
                    vent_moyen = valeurs[0]
                    vent_max = valeurs[1]
                else:
...
with col_gauche:
    ...
    # Selection sans rechargement forcé (st.rerun supprimés pour éviter la page blanche)
...
with col_droite:
    ...
    if ffvl_id:
        ...
        # Lancement de la fonction ultra-sécurisée
