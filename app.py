...
# Strictement en première instruction pour éviter les erreurs de configuration Streamlit
...
# Mise en cache pour découpler la requête réseau du rafraîchissement d'affichage Streamlit
@st.cache_data(ttl=60, show_spinner=False)
def recuperer_donnees_balise_reelles(balise_id):
    url = f"https://www.balisemeteo.com/balise.php?idBalise={balise_id}"
    fallback = {"heure": "00:00", "vent_moyen": 0.0, "vent_max": 0.0, "indice": 1, "is_fallback": True}
    try:
        req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
        # Timeout court (3s) pour éviter de geler l'application Streamlit si le serveur distant ralentit
        with urllib.request.urlopen(req, timeout=3) as response:
            ...
            # Pattern multiligne pour cibler précisément l'architecture constatée
...
with col_droite:
    ...
    if ffvl_id:
        ...
        # Utilisation d'un espace réservé pour que l'interface s'affiche même si la requête prend du temps
        conteneur_balise = st.empty()
        conteneur_balise.info("Connexion à la balise...")
        
        balise_reelle = recuperer_donnees_balise_reelles(ffvl_id)
        
        if balise_reelle.get("is_fallback", False):
            conteneur_balise.warning("⚠️ Serveur de balise injoignable ou en maintenance. Données indisponibles.")
        else:
            # Remplacement propre du texte dans le conteneur isolé
            with conteneur_balise.container():
